...
Verify that `README.md` and `review_comments/README.md` exist. This repository does not bundle proprietary manuals or regulatory source documents. If no knowledge root is configured, explain that limitation and ask the user to configure an authorized, properly licensed knowledge base; do not fabricate source-grounded results. Read [references/knowledge-map.md](references/knowledge-map.md) when locating modules or interpreting metadata.
...
1. **Complete baseline**: read the table of contents and all applicable checklist sections of `general_dd_manuals/GEN-001 通用尽调手册.md`. Preserve every applicable baseline diligence matter. Mark an item `不适用` with a reason rather than silently dropping it.
